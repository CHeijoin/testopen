...
excelPath = os.path.join(data, "testcase_tickservic.xlsx")
...
@ddt.ddt
class TS_tickservice(unittest.TestCase):
    log = Log()
    def setUp(self):
        self.log.info( "---------开始测试---------" )
        # 测试环境地址
    ...
    def test_tickservice_inf(self,values):
        u'API接口上线验证'
        self.intdiscrp = u'测试RT接口令牌无效1'
        self.log.info(values['testcaseID'] + "---------start---------"+values['testcaseID'])
        # self.url = "http://172.29.50.67:7060/sbe"
        # self.data = expath
        ...
        try:
            ...
            self.log.info(values['testcaseID'] + str(e))
            raise
        self.log.info(values['testcaseID'] + "------------end--------------------")
